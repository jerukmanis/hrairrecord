...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fair2310edit = new ew.Form("fair2310edit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fair2310edit;

    // Add fields
    var fields = currentTable.fields;
    fair2310edit.addFields([
    ...
    // Form_CustomValidate
    fair2310edit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fair2310edit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    loadjs.done("fair2310edit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fair2310edit" id="fair2310edit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_air2310_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_air2310_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="air2310" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
<?php if ($Page->bulan->Visible) { // bulan ?>
    <div id="r_bulan"<?= $Page->bulan->rowAttributes() ?>>
        <label id="elh_air2310_bulan" for="x_bulan" class="<?= $Page->LeftColumnClass ?>"><?= $Page->bulan->caption() ?><?= $Page->bulan->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->bulan->cellAttributes() ?>>
<span id="el_air2310_bulan">
<input type="<?= $Page->bulan->getInputTextType() ?>" name="x_bulan" id="x_bulan" data-table="air2310" data-field="x_bulan" value="<?= $Page->bulan->EditValue ?>" size="30" maxlength="255" placeholder="<?= HtmlEncode($Page->bulan->getPlaceHolder()) ?>"<?= $Page->bulan->editAttributes() ?> aria-describedby="x_bulan_help">
...
<?php if ($Page->unit->Visible) { // unit ?>
    <div id="r_unit"<?= $Page->unit->rowAttributes() ?>>
        <label id="elh_air2310_unit" for="x_unit" class="<?= $Page->LeftColumnClass ?>"><?= $Page->unit->caption() ?><?= $Page->unit->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->unit->cellAttributes() ?>>
<span id="el_air2310_unit">
<input type="<?= $Page->unit->getInputTextType() ?>" name="x_unit" id="x_unit" data-table="air2310" data-field="x_unit" value="<?= $Page->unit->EditValue ?>" size="30" maxlength="255" placeholder="<?= HtmlEncode($Page->unit->getPlaceHolder()) ?>"<?= $Page->unit->editAttributes() ?> aria-describedby="x_unit_help">
...
<?php if ($Page->meteran_bulansebelumnya->Visible) { // meteran_bulansebelumnya ?>
    <div id="r_meteran_bulansebelumnya"<?= $Page->meteran_bulansebelumnya->rowAttributes() ?>>
        <label id="elh_air2310_meteran_bulansebelumnya" for="x_meteran_bulansebelumnya" class="<?= $Page->LeftColumnClass ?>"><?= $Page->meteran_bulansebelumnya->caption() ?><?= $Page->meteran_bulansebelumnya->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<?php if ($Page->foto->Visible) { // foto ?>
    <div id="r_foto"<?= $Page->foto->rowAttributes() ?>>
        <label id="elh_air2310_foto" class="<?= $Page->LeftColumnClass ?>"><?= $Page->foto->caption() ?><?= $Page->foto->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<div id="fd_x_foto" class="fileinput-button ew-file-drop-zone">
    <input type="file" class="form-control ew-file-input" title="<?= $Page->foto->title() ?>" data-table="air2310" data-field="x_foto" name="x_foto" id="x_foto" lang="<?= CurrentLanguageID() ?>"<?= $Page->foto->editAttributes() ?> aria-describedby="x_foto_help"<?= ($Page->foto->ReadOnly || $Page->foto->Disabled) ? " disabled" : "" ?>>
